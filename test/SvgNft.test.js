// npx hardhat test --grep "Svg"
const { assert, expect } = require("chai")
const { network, deployments, ethers } = require("hardhat")
const { developmentChains } = require("../helper-hardhat-config")
const lowSVGImageuri =
    "data:image/svg+xml;base64,"
const highSVGimageUri =
    "data:image/svg+xml;base64,"
!developmentChains.includes(network.name)
    ? describe.skip
    : describe("SvgNFT", function () {
        let SvgNFT, MockV3Aggregator, deployer
        beforeEach(async () => {
            accounts = await ethers.getSigners()
            deployer = accounts[0]
            await deployments.fixture(["mock", "SVG"])
            SvgNFT = await ethers.getContract("SvgNFT")
            MockV3Aggregator = await ethers.getContract("MockV3Aggregator")
        })
        describe("Constructor", function () {
            it("initializes all values", async () => {
                const priceFeed = await SvgNFT.getPriceFeed()
                const highUri = await SvgNFT.getHighSVG()
                const lowUri = await SvgNFT.getLowSVG()
                const tokenCounter = await SvgNFT.getTokenCounter()
                assert.equal(priceFeed, MockV3Aggregator.address)
                assert.equal(highUri, highSVGimageUri)
                assert.equal(lowUri, lowSVGImageuri)
                assert.equal(tokenCounter, "0")
            })
        })
        describe("Mint NFT", async () => {
            it("emits event CreatedNFT and increases token counter", async () => {
                return new Promise(async (resolve, reject) => {
                    SvgNFT.once("CreatedNFT", async () => {
                        try {
                            const tokenCounter = await SvgNFT.getTokenCounter()
                            assert.equal(tokenCounter, "1")
                            resolve()
                        } catch (error) {
                            reject(error)
                        }
                    })
                    try {
                        const value = ethers.utils.parseEther("1")
                        await expect(SvgNFT.mintNft(value)).to.emit(
                            SvgNFT,
                            "CreatedNFT"
                        )
                    } catch (error) {
                        reject(error)
                    }
                })
            })
        })
    })